...
%extend neueda::@{venue.lower()}Codec {
    ...
    @for m in message_data:
    static const @{venue}@{m}* pointerTo@{m}(const void* buffer, size_t lenth) {
        // AUTO-GEN TODO
        // if length < sizeof (@{venue}@{m})
        return reinterpret_cast<const neueda::@{venue}@{m}*>(buffer);
    }
