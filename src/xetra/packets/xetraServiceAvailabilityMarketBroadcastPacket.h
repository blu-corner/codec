...
class xetraServiceAvailabilityMarketBroadcastPacket
{
    public:
        ...
        codecState serialize (void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mSelectiveRequestForQuoteServiceTradeDate, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mSelectiveRequestForQuoteServiceStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        codecState deserialize (const void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mSelectiveRequestForQuoteServiceTradeDate, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mSelectiveRequestForQuoteServiceStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
