    ...
    public:
        ...
        size_t getRawSize () const
        {
            size_t result = mMessageHeaderOut.getRawSize()
                ...
                + sizeof (mFreeText4)
                + sizeof (mPad7)
                + xetra::getRawSize (mOrderBookItemGrp);
            return result;
        }
        ...
        codecState serialize (void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mTrdRptStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mMessageEventSource, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mSide, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mAllocMethod, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mNoOrderBookItems, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mRootPartyEnteringTrader, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mFirmTradeID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        codecState deserialize (const void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mTrdRptStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mMessageEventSource, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mSide, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mAllocMethod, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mNoOrderBookItems, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mRootPartyEnteringTrader, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mFirmTradeID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        string toString () const
        {
            ...
            sss << "XetraEnLightCreateDealNotification::"
                ...
                << "[Pad7=" << getPad7 () << "],"
                << "[OrderBookItemGrp=" << xetra::toString (getOrderBookItemGrp ()) << "]";
            return sss.str();
        }
};
