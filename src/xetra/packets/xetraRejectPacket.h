 ...
 */
#ifndef XETRA_REJECT_PACKET_H
#define XETRA_REJECT_PACKET_H
...
class xetraRejectPacket
{
    public:
        ...
        codecState serialize (void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mSessionRejectReason, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mVarTextLen, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mSessionStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        codecState deserialize (const void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mSessionRejectReason, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mVarTextLen, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mSessionStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
...
const uint32_t xetraRejectPacket::SESSION_REJECT_REASON_MIN = 0;
const uint32_t xetraRejectPacket::SESSION_REJECT_REASON_MAX = 4294967294;
const uint32_t xetraRejectPacket::SESSION_REJECT_REASON_NO_VALUE = 0xFFFFFFFF;
const int16_t xetraRejectPacket::VAR_TEXT_LEN_MIN = 0;
const int16_t xetraRejectPacket::VAR_TEXT_LEN_MAX = 2000;
const int16_t xetraRejectPacket::VAR_TEXT_LEN_NO_VALUE = 0xFFFF;
const int8_t xetraRejectPacket::SESSION_STATUS_MIN = 0;
const int8_t xetraRejectPacket::SESSION_STATUS_MAX = 4;
const int8_t xetraRejectPacket::SESSION_STATUS_NO_VALUE = 0xFF;
const char xetraRejectPacket::PAD1_NO_VALUE[1] = {0x00};
const size_t xetraRejectPacket::PAD1_MAX_LENGTH = 1;
const size_t xetraRejectPacket::VAR_TEXT_MAX_LENGTH = 2000;


} // namespace neueda

#endif // XETRA_REJECT_PACKET_H
