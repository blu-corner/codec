...
class xetraPartyDetailsGrpCompPacket
{
    public:
        ...
        {
            codecState state;
            state = xetra::serialize (mPartyDetailIDExecutingTrader, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mPartyDetailExecutingTrader, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mPartyDetailRoleQualifier, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mPartyDetailStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mPartyDetailDeskID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        {
            codecState state;
            state = xetra::deserialize (mPartyDetailIDExecutingTrader, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mPartyDetailExecutingTrader, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mPartyDetailRoleQualifier, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mPartyDetailStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mPartyDetailDeskID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
