 ...
 */
#ifndef XETRA_PARTYACTIONREPORT_PACKET_H
#define XETRA_PARTYACTIONREPORT_PACKET_H
...
class xetraPartyActionReportPacket
{
    public:
        // no value constants
        ...
        codecState serialize (void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mRequestingPartyIDExecutingTrader, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mPartyIDExecutingUnit, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mPartyIDExecutingTrader, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mRequestingPartyIDExecutingSystem, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mMarketID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mPartyActionType, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::serialize (mRequestingPartyIDEnteringFirm, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        codecState deserialize (const void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mRequestingPartyIDExecutingTrader, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mPartyIDExecutingUnit, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mPartyIDExecutingTrader, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mRequestingPartyIDExecutingSystem, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mMarketID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mPartyActionType, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = xetra::deserialize (mRequestingPartyIDEnteringFirm, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            mMessageHeaderOut.mBodyLen = getRawSize ();
            return GW_CODEC_SUCCESS;
        }
...
const uint64_t xetraPartyActionReportPacket::TRANSACT_TIME_MIN = 0UL;
const uint64_t xetraPartyActionReportPacket::TRANSACT_TIME_MAX = 18446744073709551614UL;
const uint64_t xetraPartyActionReportPacket::TRANSACT_TIME_NO_VALUE = 0xFFFFFFFFFFFFFFFF;
const int32_t xetraPartyActionReportPacket::TRADE_DATE_MIN = 0;
const int32_t xetraPartyActionReportPacket::TRADE_DATE_MAX = 99991231;
const int32_t xetraPartyActionReportPacket::TRADE_DATE_NO_VALUE = 0xFFFFFFFF;
const uint32_t xetraPartyActionReportPacket::REQUESTING_PARTY_IDEXECUTING_TRADER_MIN = 0;
const uint32_t xetraPartyActionReportPacket::REQUESTING_PARTY_IDEXECUTING_TRADER_MAX = 4294967294;
const uint32_t xetraPartyActionReportPacket::REQUESTING_PARTY_IDEXECUTING_TRADER_NO_VALUE = 0xFFFFFFFF;
const uint32_t xetraPartyActionReportPacket::PARTY_IDEXECUTING_UNIT_MIN = 0;
const uint32_t xetraPartyActionReportPacket::PARTY_IDEXECUTING_UNIT_MAX = 4294967294;
const uint32_t xetraPartyActionReportPacket::PARTY_IDEXECUTING_UNIT_NO_VALUE = 0xFFFFFFFF;
const uint32_t xetraPartyActionReportPacket::PARTY_IDEXECUTING_TRADER_MIN = 0;
const uint32_t xetraPartyActionReportPacket::PARTY_IDEXECUTING_TRADER_MAX = 4294967294;
const uint32_t xetraPartyActionReportPacket::PARTY_IDEXECUTING_TRADER_NO_VALUE = 0xFFFFFFFF;
const uint32_t xetraPartyActionReportPacket::REQUESTING_PARTY_IDEXECUTING_SYSTEM_MIN = 0;
const uint32_t xetraPartyActionReportPacket::REQUESTING_PARTY_IDEXECUTING_SYSTEM_MAX = 4294967294;
const uint32_t xetraPartyActionReportPacket::REQUESTING_PARTY_IDEXECUTING_SYSTEM_NO_VALUE = 0xFFFFFFFF;
const int16_t xetraPartyActionReportPacket::MARKET_ID_MIN = 1;
const int16_t xetraPartyActionReportPacket::MARKET_ID_MAX = 255;
const int16_t xetraPartyActionReportPacket::MARKET_ID_NO_VALUE = 0xFFFF;
const int8_t xetraPartyActionReportPacket::PARTY_ACTION_TYPE_MIN = 1;
const int8_t xetraPartyActionReportPacket::PARTY_ACTION_TYPE_MAX = 2;
const int8_t xetraPartyActionReportPacket::PARTY_ACTION_TYPE_NO_VALUE = 0xFF;
const int8_t xetraPartyActionReportPacket::REQUESTING_PARTY_IDENTERING_FIRM_MIN = 1;
const int8_t xetraPartyActionReportPacket::REQUESTING_PARTY_IDENTERING_FIRM_MAX = 2;
const int8_t xetraPartyActionReportPacket::REQUESTING_PARTY_IDENTERING_FIRM_NO_VALUE = 0xFF;


} // namespace neueda

#endif // XETRA_PARTYACTIONREPORT_PACKET_H
