...
CODEC_CPP_GET_GROUP = '''\
static void
get{GROUP} (cdr &d, const xetra{GROUP}Packet& packet)
{{
{CODE}
}}
'''
...
void
xetraCodec::get{MESSAGE} (cdr &d, const void *buf, size_t len, size_t& used)
{{
    xetra{MESSAGE}Packet packet;
    codecState state = packet.deserialize (buf, len, used);
    if (state != GW_CODEC_SUCCESS)
        throw xetra::codecError(state, "cannot deserialize {MESSAGE}");
...
CODEC_CPP_PUT_GROUP = '''\
static void
put{GROUP} (const cdr &d, xetra{GROUP}Packet& packet)
{{
{CODE}
}}
'''
...
void
xetraCodec::put{MESSAGE} (const cdr &d, void *buf, size_t len, size_t& used)
{{
    xetra{MESSAGE}Packet packet;
{CODE}
    codecState state = packet.serialize (buf, len, used);
    if (state != GW_CODEC_SUCCESS)
        throw xetra::codecError(state, "cannot serialize {MESSAGE}");
}}
'''
...
def render_put_fields(struct, venue, number_of_spaces=4):
	...
	for field in struct.findall('field'):
		if field.attrib['class'] == 'sequence':
			...
			result += '{0}xetra::getInteger<{1}, {2}, &{2}::set{3}, &{2}::reset{3}> (d, {3}, packet);\n'.format(spaces, field.attrib['type'], get_typename(struct.attrib['name']), field.attrib['name'])
		elif field.attrib['class'] == 'double':
			result += '{0}xetra::getDouble<{1}, &{1}::set{2}, &{1}::reset{2}> (d, {2}, packet);\n'.format(spaces, get_typename(struct.attrib['name']), field.attrib['name'])
		elif field.attrib['class'] == 'string':
			result += '{0}xetra::getString<{1}, &{1}::set{2}, &{1}::reset{2}> (d, {2}, packet);\n'.format(spaces, get_typename(struct.attrib['name']), field.attrib['name'])
		elif field.attrib['class'] == 'varstring':
			result += '{0}xetra::getString<{1}, &{1}::set{2}, &{1}::reset{2}> (d, {2}, packet);\n'.format(spaces, get_typename(struct.attrib['name']), field.attrib['name'])
		else:
			raise Exception('unknown class ' + field.attrib['class'])
