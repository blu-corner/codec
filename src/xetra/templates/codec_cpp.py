...
{{
    ...
    if (state != GW_CODEC_SUCCESS)
        throw {EXCH}::codecError(state, "cannot deserialize {MESSAGE}");
...
{CODE}
    ...
    if (state != GW_CODEC_SUCCESS)
        throw {EXCH}::codecError(state, "cannot serialize {MESSAGE}");
}}
'''
...
{SWITCH}
            default:
                ...
                throw {EXCH}::codecError (GW_CODEC_ERROR, ss.str ());
        }}
    }}
    catch ({EXCH}::codecError &e)
    {{
...
def render_get_fields(group, venue):
	...
	for field in group.findall('field'):
		if field.attrib['class'] == 'sequence':
			...
			result += '{0}{1}::setInteger<{2}, {3}, &{3}::get{4}, &{3}::is{4}Valid> (d, {4}, packet);\n'.format(spaces, venue.attrib['name'].lower(), field.attrib['type'], get_typename(venue.attrib['name'].lower(), group.attrib['name']), field.attrib['name'])
		elif field.attrib['class'] == 'double':
			result += '{0}{1}::setDouble<{2}, &{2}::get{3}, &{2}::is{3}Valid> (d, {3}, packet);\n'.format(spaces, venue.attrib['name'].lower(), get_typename(venue.attrib['name'].lower(), group.attrib['name']), field.attrib['name'])
		elif field.attrib['class'] == 'string':
			result += '{0}{1}::setString<{2}, &{2}::get{3}, &{2}::is{3}Valid> (d, {3}, packet);\n'.format(spaces, venue.attrib['name'].lower(), get_typename(venue.attrib['name'].lower(), group.attrib['name']), field.attrib['name'])
...
def render_put_fields(struct, venue, number_of_spaces=4):
	...
	for field in struct.findall('field'):
		if field.attrib['class'] == 'sequence':
			...
			result += '{0}{1}::getInteger<{2}, {3}, &{3}::set{4}, &{3}::reset{4}> (d, {4}, packet);\n'.format(spaces, venue.attrib['name'].lower(), field.attrib['type'], get_typename(venue.attrib['name'].lower(), struct.attrib['name']), field.attrib['name'])
		elif field.attrib['class'] == 'double':
			result += '{0}{1}::getDouble<{2}, &{2}::set{3}, &{2}::reset{3}> (d, {3}, packet);\n'.format(spaces, venue.attrib['name'].lower(), get_typename(venue.attrib['name'].lower(), struct.attrib['name']), field.attrib['name'])
		elif field.attrib['class'] == 'string':
			result += '{0}{1}::getString<{2}, &{2}::set{3}, &{2}::reset{3}> (d, {3}, packet);\n'.format(spaces, venue.attrib['name'].lower(), get_typename(venue.attrib['name'].lower(), struct.attrib['name']), field.attrib['name'])
		elif field.attrib['class'] == 'varstring':
			result += '{0}{1}::getString<{2}, &{2}::set{3}, &{2}::reset{3}> (d, {3}, packet);\n'.format(spaces, venue.attrib['name'].lower(), get_typename(venue.attrib['name'].lower(), struct.attrib['name']), field.attrib['name'])
		else:
			raise Exception('unknown class ' + field.attrib['class'])
