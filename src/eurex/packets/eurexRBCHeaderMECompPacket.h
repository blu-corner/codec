 ...
 */
#ifndef EUREX_RBCHEADERMECOMP_PACKET_H
#define EUREX_RBCHEADERMECOMP_PACKET_H
...
class eurexRBCHeaderMECompPacket
{
    public:
        // no value constants
        ...
        codecState serialize (void *buf, size_t len, size_t &used)
        {
            ...
            state = eurex::serialize (mNotificationIn, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mSendingTime, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mApplSubID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mApplResendFlag, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        codecState deserialize (const void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mNotificationIn, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mSendingTime, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mApplSubID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mApplResendFlag, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
...
const uint64_t eurexRBCHeaderMECompPacket::TRD_REG_TSTIME_OUT_MIN = 0UL;
const uint64_t eurexRBCHeaderMECompPacket::TRD_REG_TSTIME_OUT_MAX = 18446744073709551614UL;
const uint64_t eurexRBCHeaderMECompPacket::TRD_REG_TSTIME_OUT_NO_VALUE = 0xFFFFFFFFFFFFFFFF;
const uint64_t eurexRBCHeaderMECompPacket::NOTIFICATION_IN_MIN = 0UL;
const uint64_t eurexRBCHeaderMECompPacket::NOTIFICATION_IN_MAX = 18446744073709551614UL;
const uint64_t eurexRBCHeaderMECompPacket::NOTIFICATION_IN_NO_VALUE = 0xFFFFFFFFFFFFFFFF;
const uint64_t eurexRBCHeaderMECompPacket::SENDING_TIME_MIN = 0UL;
const uint64_t eurexRBCHeaderMECompPacket::SENDING_TIME_MAX = 18446744073709551614UL;
const uint64_t eurexRBCHeaderMECompPacket::SENDING_TIME_NO_VALUE = 0xFFFFFFFFFFFFFFFF;
const uint32_t eurexRBCHeaderMECompPacket::APPL_SUB_ID_MIN = 0;
const uint32_t eurexRBCHeaderMECompPacket::APPL_SUB_ID_MAX = 4294967294;
const uint32_t eurexRBCHeaderMECompPacket::APPL_SUB_ID_NO_VALUE = 0xFFFFFFFF;
const uint16_t eurexRBCHeaderMECompPacket::PARTITION_ID_MIN = 0;
const uint16_t eurexRBCHeaderMECompPacket::PARTITION_ID_MAX = 65534;
const uint16_t eurexRBCHeaderMECompPacket::PARTITION_ID_NO_VALUE = 0xFFFF;
const char eurexRBCHeaderMECompPacket::APPL_MSG_ID_NO_VALUE[16] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const size_t eurexRBCHeaderMECompPacket::APPL_MSG_ID_MAX_LENGTH = 16;
const int8_t eurexRBCHeaderMECompPacket::APPL_ID_MIN = 0;
const int8_t eurexRBCHeaderMECompPacket::APPL_ID_MAX = 10;
const int8_t eurexRBCHeaderMECompPacket::APPL_ID_NO_VALUE = 0xFF;
const int8_t eurexRBCHeaderMECompPacket::APPL_RESEND_FLAG_MIN = 0;
const int8_t eurexRBCHeaderMECompPacket::APPL_RESEND_FLAG_MAX = 1;
const int8_t eurexRBCHeaderMECompPacket::APPL_RESEND_FLAG_NO_VALUE = 0xFF;
const int8_t eurexRBCHeaderMECompPacket::LAST_FRAGMENT_MIN = 0;
const int8_t eurexRBCHeaderMECompPacket::LAST_FRAGMENT_MAX = 1;
const int8_t eurexRBCHeaderMECompPacket::LAST_FRAGMENT_NO_VALUE = 0xFF;
const char eurexRBCHeaderMECompPacket::PAD7_NO_VALUE[7] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const size_t eurexRBCHeaderMECompPacket::PAD7_MAX_LENGTH = 7;


} // namespace neueda

#endif // EUREX_RBCHEADERMECOMP_PACKET_H
