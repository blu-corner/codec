 ...
 */
#ifndef EUREX_LOGONREQUEST_PACKET_H
#define EUREX_LOGONREQUEST_PACKET_H
...
class eurexLogonRequestPacket
{
    public:
        ...
        codecState serialize (void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mHeartBtInt, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mPartyIDSessionID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mDefaultCstmApplVerID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mPassword, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mApplUsageOrders, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mApplUsageQuotes, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mOrderRoutingIndicator, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mFIXEngineName, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mFIXEngineVersion, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mFIXEngineVendor, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mApplicationSystemName, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mApplicationSystemVersion, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mApplicationSystemVendor, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        codecState deserialize (const void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mHeartBtInt, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mPartyIDSessionID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mDefaultCstmApplVerID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mPassword, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mApplUsageOrders, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mApplUsageQuotes, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mOrderRoutingIndicator, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mFIXEngineName, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mFIXEngineVersion, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mFIXEngineVendor, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mApplicationSystemName, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mApplicationSystemVersion, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mApplicationSystemVendor, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
...
const uint32_t eurexLogonRequestPacket::HEART_BT_INT_MIN = 0;
const uint32_t eurexLogonRequestPacket::HEART_BT_INT_MAX = 4294967294;
const uint32_t eurexLogonRequestPacket::HEART_BT_INT_NO_VALUE = 0xFFFFFFFF;
const uint32_t eurexLogonRequestPacket::PARTY_IDSESSION_ID_MIN = 0;
const uint32_t eurexLogonRequestPacket::PARTY_IDSESSION_ID_MAX = 4294967294;
const uint32_t eurexLogonRequestPacket::PARTY_IDSESSION_ID_NO_VALUE = 0xFFFFFFFF;
const char eurexLogonRequestPacket::DEFAULT_CSTM_APPL_VER_ID_NO_VALUE[30] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const size_t eurexLogonRequestPacket::DEFAULT_CSTM_APPL_VER_ID_MAX_LENGTH = 30;
const char eurexLogonRequestPacket::PASSWORD_NO_VALUE[32] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const size_t eurexLogonRequestPacket::PASSWORD_MAX_LENGTH = 32;
const char eurexLogonRequestPacket::APPL_USAGE_ORDERS_NO_VALUE[1] = {0x00};
const size_t eurexLogonRequestPacket::APPL_USAGE_ORDERS_MAX_LENGTH = 1;
const char eurexLogonRequestPacket::APPL_USAGE_QUOTES_NO_VALUE[1] = {0x00};
const size_t eurexLogonRequestPacket::APPL_USAGE_QUOTES_MAX_LENGTH = 1;
const char eurexLogonRequestPacket::ORDER_ROUTING_INDICATOR_NO_VALUE[1] = {0x00};
const size_t eurexLogonRequestPacket::ORDER_ROUTING_INDICATOR_MAX_LENGTH = 1;
const char eurexLogonRequestPacket::FIXENGINE_NAME_NO_VALUE[30] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const size_t eurexLogonRequestPacket::FIXENGINE_NAME_MAX_LENGTH = 30;
const char eurexLogonRequestPacket::FIXENGINE_VERSION_NO_VALUE[30] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const size_t eurexLogonRequestPacket::FIXENGINE_VERSION_MAX_LENGTH = 30;
const char eurexLogonRequestPacket::FIXENGINE_VENDOR_NO_VALUE[30] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const size_t eurexLogonRequestPacket::FIXENGINE_VENDOR_MAX_LENGTH = 30;
const char eurexLogonRequestPacket::APPLICATION_SYSTEM_NAME_NO_VALUE[30] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const size_t eurexLogonRequestPacket::APPLICATION_SYSTEM_NAME_MAX_LENGTH = 30;
const char eurexLogonRequestPacket::APPLICATION_SYSTEM_VERSION_NO_VALUE[30] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const size_t eurexLogonRequestPacket::APPLICATION_SYSTEM_VERSION_MAX_LENGTH = 30;
const char eurexLogonRequestPacket::APPLICATION_SYSTEM_VENDOR_NO_VALUE[30] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const size_t eurexLogonRequestPacket::APPLICATION_SYSTEM_VENDOR_MAX_LENGTH = 30;
const char eurexLogonRequestPacket::PAD3_NO_VALUE[3] = {0x00, 0x00, 0x00};
const size_t eurexLogonRequestPacket::PAD3_MAX_LENGTH = 3;


} // namespace neueda

#endif // EUREX_LOGONREQUEST_PACKET_H
