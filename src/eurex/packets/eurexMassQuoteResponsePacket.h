    ...
    public:
        ...
        size_t getRawSize () const
        {
            size_t result = mMessageHeaderOut.getRawSize()
                ...
                + sizeof (mPad3)
                + eurex::getRawSize (mQuoteEntryAckGrp);
            return result;
        }
        ...
        codecState serialize (void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mQuoteResponseID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mMarketSegmentID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mNoQuoteEntries, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        codecState deserialize (const void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mQuoteResponseID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mMarketSegmentID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mNoQuoteEntries, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        string toString () const
        {
            ...
            sss << "MassQuoteResponse::"
                ...
                << "[Pad3=" << getPad3 () << "],"
                << "[QuoteEntryAckGrp=" << eurex::toString (getQuoteEntryAckGrp ()) << "]";
            return sss.str();
        }
};
