 ...
 */
#ifndef EUREX_PARTYENTITLEMENTSUPDATEREPORT_PACKET_H
#define EUREX_PARTYENTITLEMENTSUPDATEREPORT_PACKET_H
...
class eurexPartyEntitlementsUpdateReportPacket
{
    public:
        // no value constants
        ...
        // constructor
        eurexPartyEntitlementsUpdateReportPacket ()
        {
        ...
        codecState serialize (void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mPartyDetailIDExecutingUnit, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mRequestingPartyIDExecutingSystem, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mMarketID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mListUpdateAction, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mRequestingPartyEnteringFirm, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mRequestingPartyClearingFirm, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        codecState deserialize (const void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mPartyDetailIDExecutingUnit, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mRequestingPartyIDExecutingSystem, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mMarketID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mListUpdateAction, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mRequestingPartyEnteringFirm, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mRequestingPartyClearingFirm, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
...
const uint64_t eurexPartyEntitlementsUpdateReportPacket::TRANSACT_TIME_MIN = 0UL;
const uint64_t eurexPartyEntitlementsUpdateReportPacket::TRANSACT_TIME_MAX = 18446744073709551614UL;
const uint64_t eurexPartyEntitlementsUpdateReportPacket::TRANSACT_TIME_NO_VALUE = 0xFFFFFFFFFFFFFFFF;
const int32_t eurexPartyEntitlementsUpdateReportPacket::TRADE_DATE_MIN = 0;
const int32_t eurexPartyEntitlementsUpdateReportPacket::TRADE_DATE_MAX = 99991231;
const int32_t eurexPartyEntitlementsUpdateReportPacket::TRADE_DATE_NO_VALUE = 0xFFFFFFFF;
const uint32_t eurexPartyEntitlementsUpdateReportPacket::PARTY_DETAIL_IDEXECUTING_UNIT_MIN = 0;
const uint32_t eurexPartyEntitlementsUpdateReportPacket::PARTY_DETAIL_IDEXECUTING_UNIT_MAX = 4294967294;
const uint32_t eurexPartyEntitlementsUpdateReportPacket::PARTY_DETAIL_IDEXECUTING_UNIT_NO_VALUE = 0xFFFFFFFF;
const uint32_t eurexPartyEntitlementsUpdateReportPacket::REQUESTING_PARTY_IDEXECUTING_SYSTEM_MIN = 0;
const uint32_t eurexPartyEntitlementsUpdateReportPacket::REQUESTING_PARTY_IDEXECUTING_SYSTEM_MAX = 4294967294;
const uint32_t eurexPartyEntitlementsUpdateReportPacket::REQUESTING_PARTY_IDEXECUTING_SYSTEM_NO_VALUE = 0xFFFFFFFF;
const int16_t eurexPartyEntitlementsUpdateReportPacket::MARKET_ID_MIN = 1;
const int16_t eurexPartyEntitlementsUpdateReportPacket::MARKET_ID_MAX = 255;
const int16_t eurexPartyEntitlementsUpdateReportPacket::MARKET_ID_NO_VALUE = 0xFFFF;
const char eurexPartyEntitlementsUpdateReportPacket::LIST_UPDATE_ACTION_NO_VALUE[1] = {0x00};
const size_t eurexPartyEntitlementsUpdateReportPacket::LIST_UPDATE_ACTION_MAX_LENGTH = 1;
const char eurexPartyEntitlementsUpdateReportPacket::REQUESTING_PARTY_ENTERING_FIRM_NO_VALUE[9] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const size_t eurexPartyEntitlementsUpdateReportPacket::REQUESTING_PARTY_ENTERING_FIRM_MAX_LENGTH = 9;
const char eurexPartyEntitlementsUpdateReportPacket::REQUESTING_PARTY_CLEARING_FIRM_NO_VALUE[9] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const size_t eurexPartyEntitlementsUpdateReportPacket::REQUESTING_PARTY_CLEARING_FIRM_MAX_LENGTH = 9;
const int8_t eurexPartyEntitlementsUpdateReportPacket::PARTY_DETAIL_STATUS_MIN = 0;
const int8_t eurexPartyEntitlementsUpdateReportPacket::PARTY_DETAIL_STATUS_MAX = 1;
const int8_t eurexPartyEntitlementsUpdateReportPacket::PARTY_DETAIL_STATUS_NO_VALUE = 0xFF;
const char eurexPartyEntitlementsUpdateReportPacket::PAD6_NO_VALUE[6] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const size_t eurexPartyEntitlementsUpdateReportPacket::PAD6_MAX_LENGTH = 6;


} // namespace neueda

#endif // EUREX_PARTYENTITLEMENTSUPDATEREPORT_PACKET_H
