 ...
 */
#ifndef EUREX_HEADER_PACKET_H
#define EUREX_HEADER_PACKET_H
...
PACKED(class eurexHeaderPacket
{
    public:
        // no value constants
        ...
        codecState deserialize (const void *buf, size_t len, size_t &used)
        {
            codecState state;
            uint32_t vBodyLen;
            state = eurex::deserialize (vBodyLen, buf, len, used);
            mBodyLen = vBodyLen;
            if (state != GW_CODEC_SUCCESS) return state;
            uint16_t vTemplateID;
            state = eurex::deserialize (vTemplateID, buf, len, used);
...
const uint32_t eurexHeaderPacket::BODY_LEN_MIN = 0;
const uint32_t eurexHeaderPacket::BODY_LEN_MAX = 4294967294;
const uint32_t eurexHeaderPacket::BODY_LEN_NO_VALUE = 0xFFFFFFFF;
const uint16_t eurexHeaderPacket::TEMPLATE_ID_MIN = 0;
const uint16_t eurexHeaderPacket::TEMPLATE_ID_MAX = 65534;
const uint16_t eurexHeaderPacket::TEMPLATE_ID_NO_VALUE = 0xFFFF;


} // namespace neueda

#endif // EUREX_HEADER_PACKET_H
