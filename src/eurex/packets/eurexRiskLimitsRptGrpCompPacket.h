...
class eurexRiskLimitsRptGrpCompPacket
{
    public:
        ...
        {
            codecState state;
            state = eurex::serialize (mRiskLimitQty, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mRiskLimitOpenQty, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mRiskLimitNetPositionQty, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mRiskLimitType, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mRiskLimitRequestingPartyRole, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mRiskLimitViolationIndicator, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        {
            codecState state;
            state = eurex::deserialize (mRiskLimitQty, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mRiskLimitOpenQty, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mRiskLimitNetPositionQty, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mRiskLimitType, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mRiskLimitRequestingPartyRole, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mRiskLimitViolationIndicator, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
