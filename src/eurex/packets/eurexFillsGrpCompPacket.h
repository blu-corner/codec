 ...
 */
#ifndef EUREX_FILLSGRPCOMP_PACKET_H
#define EUREX_FILLSGRPCOMP_PACKET_H
...
class eurexFillsGrpCompPacket
{
    public:
        ...
        {
            codecState state;
            state = eurex::serialize (mFillPx, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mFillQty, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mFillMatchID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mFillExecID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mFillLiquidityInd, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        {
            codecState state;
            state = eurex::deserialize (mFillPx, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mFillQty, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mFillMatchID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mFillExecID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mFillLiquidityInd, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
...
const double eurexFillsGrpCompPacket::FILL_PX_MIN = -92233720368.54775807;
const double eurexFillsGrpCompPacket::FILL_PX_MAX = 92233720368.54775807;
const int64_t eurexFillsGrpCompPacket::FILL_PX_NO_VALUE = 0x8000000000000000;
const double eurexFillsGrpCompPacket::FILL_QTY_MIN = -922337203685477.5807;
const double eurexFillsGrpCompPacket::FILL_QTY_MAX = 922337203685477.5807;
const int64_t eurexFillsGrpCompPacket::FILL_QTY_NO_VALUE = 0x8000000000000000;
const uint32_t eurexFillsGrpCompPacket::FILL_MATCH_ID_MIN = 0;
const uint32_t eurexFillsGrpCompPacket::FILL_MATCH_ID_MAX = 4294967294;
const uint32_t eurexFillsGrpCompPacket::FILL_MATCH_ID_NO_VALUE = 0xFFFFFFFF;
const int32_t eurexFillsGrpCompPacket::FILL_EXEC_ID_MIN = -2147483647;
const int32_t eurexFillsGrpCompPacket::FILL_EXEC_ID_MAX = 2147483647;
const int32_t eurexFillsGrpCompPacket::FILL_EXEC_ID_NO_VALUE = 0x80000000;
const int8_t eurexFillsGrpCompPacket::FILL_LIQUIDITY_IND_MIN = 1;
const int8_t eurexFillsGrpCompPacket::FILL_LIQUIDITY_IND_MAX = 7;
const int8_t eurexFillsGrpCompPacket::FILL_LIQUIDITY_IND_NO_VALUE = 0xFF;
const char eurexFillsGrpCompPacket::PAD7_NO_VALUE[7] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const size_t eurexFillsGrpCompPacket::PAD7_MAX_LENGTH = 7;


} // namespace neueda

#endif // EUREX_FILLSGRPCOMP_PACKET_H
