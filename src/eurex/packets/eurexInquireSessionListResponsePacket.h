 ...
 */
#ifndef EUREX_INQUIRESESSIONLISTRESPONSE_PACKET_H
#define EUREX_INQUIRESESSIONLISTRESPONSE_PACKET_H
...
class eurexInquireSessionListResponsePacket
{
    public:
        // no value constants
        ...
        int16_t mNoSessions;
        char mPad6[6];
        vector<eurexSessionsGrpCompPacket> mSessionsGrp;

        // constructor
        eurexInquireSessionListResponsePacket ()
        {
        ...
        const vector<eurexSessionsGrpCompPacket>& getSessionsGrp () const
        {
            return mSessionsGrp;
        }

        bool setSessionsGrp (const vector<eurexSessionsGrpCompPacket>& v)
        {
            mSessionsGrp = v;
            mNoSessions = v.size ();
        ...
        codecState serialize (void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mNoSessions, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        codecState deserialize (const void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mNoSessions, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mPad6, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            mSessionsGrp.resize (mNoSessions);
            for (vector<eurexSessionsGrpCompPacket>::iterator it = mSessionsGrp.begin (); it != mSessionsGrp.end (); ++it)
            {
        ...
        string toString () const
        {
            ...
            sss << "InquireSessionListResponse::"
                ...
                << "[Pad6=" << getPad6 () << "],"
                << "[SessionsGrp=" << eurex::toString (getSessionsGrp ()) << "]";
            return sss.str();
        }
};

const int16_t eurexInquireSessionListResponsePacket::NO_SESSIONS_MIN = 1;
const int16_t eurexInquireSessionListResponsePacket::NO_SESSIONS_MAX = 1000;
const int16_t eurexInquireSessionListResponsePacket::NO_SESSIONS_NO_VALUE = 0xFFFF;
const char eurexInquireSessionListResponsePacket::PAD6_NO_VALUE[6] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const size_t eurexInquireSessionListResponsePacket::PAD6_MAX_LENGTH = 6;
const size_t eurexInquireSessionListResponsePacket::SESSIONS_GRP_MIN = 1;
const size_t eurexInquireSessionListResponsePacket::SESSIONS_GRP_MAX = 1000;


} // namespace neueda

#endif // EUREX_INQUIRESESSIONLISTRESPONSE_PACKET_H
