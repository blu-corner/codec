 ...
 */
#ifndef EUREX_INSTRMNTLEGEXECGRPCOMP_PACKET_H
#define EUREX_INSTRMNTLEGEXECGRPCOMP_PACKET_H
...
class eurexInstrmntLegExecGrpCompPacket
{
    public:
        // no value constants
        ...
        static const int8_t FILL_REF_ID_MIN;
        static const int8_t FILL_REF_ID_MAX;
        static const int8_t FILL_REF_ID_NO_VALUE;
        ...
        // constructor
        eurexInstrmntLegExecGrpCompPacket ()
        {
            ...
            mFillRefID = FILL_REF_ID_NO_VALUE;
        ...
        int8_t getFillRefID () const
        {
            return mFillRefID;
        }

        bool setFillRefID (int8_t v)
        {
            mFillRefID = v;
            return ((FILL_REF_ID_MIN <= mFillRefID && mFillRefID <= FILL_REF_ID_MAX) || mFillRefID == FILL_REF_ID_NO_VALUE);
        }

        bool isFillRefIDValid () const
        {
            return (mFillRefID != FILL_REF_ID_NO_VALUE);
        }
        ...
        codecState serialize (void *buf, size_t len, size_t &used)
        {
            ...
            state = eurex::serialize (mLegLastPx, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mLegLastQty, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mLegExecID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mLegSide, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mFillRefID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        codecState deserialize (const void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mLegLastPx, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mLegLastQty, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mLegExecID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mLegSide, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mFillRefID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        {
            stringstream sss;
            sss << "InstrmntLegExecGrpComp::"
                << "[LegSecurityID=" << getLegSecurityID () << "],"
                << "[LegLastPx=" << getLegLastPx () << "],"
                << "[LegLastQty=" << getLegLastQty () << "],"
                << "[LegExecID=" << getLegExecID () << "],"
                << "[LegSide=" << getLegSide () << "],"
                << "[FillRefID=" << getFillRefID () << "],"
                << "[Pad2=" << getPad2 () << "]";
            return sss.str();
        }
};

const int64_t eurexInstrmntLegExecGrpCompPacket::LEG_SECURITY_ID_MIN = -9223372036854775807L;
const int64_t eurexInstrmntLegExecGrpCompPacket::LEG_SECURITY_ID_MAX = 9223372036854775807L;
const int64_t eurexInstrmntLegExecGrpCompPacket::LEG_SECURITY_ID_NO_VALUE = 0x8000000000000000;
const double eurexInstrmntLegExecGrpCompPacket::LEG_LAST_PX_MIN = -92233720368.54775807;
const double eurexInstrmntLegExecGrpCompPacket::LEG_LAST_PX_MAX = 92233720368.54775807;
const int64_t eurexInstrmntLegExecGrpCompPacket::LEG_LAST_PX_NO_VALUE = 0x8000000000000000;
const double eurexInstrmntLegExecGrpCompPacket::LEG_LAST_QTY_MIN = -922337203685477.5807;
const double eurexInstrmntLegExecGrpCompPacket::LEG_LAST_QTY_MAX = 922337203685477.5807;
const int64_t eurexInstrmntLegExecGrpCompPacket::LEG_LAST_QTY_NO_VALUE = 0x8000000000000000;
const int32_t eurexInstrmntLegExecGrpCompPacket::LEG_EXEC_ID_MIN = -2147483647;
const int32_t eurexInstrmntLegExecGrpCompPacket::LEG_EXEC_ID_MAX = 2147483647;
const int32_t eurexInstrmntLegExecGrpCompPacket::LEG_EXEC_ID_NO_VALUE = 0x80000000;
const int8_t eurexInstrmntLegExecGrpCompPacket::LEG_SIDE_MIN = 1;
const int8_t eurexInstrmntLegExecGrpCompPacket::LEG_SIDE_MAX = 2;
const int8_t eurexInstrmntLegExecGrpCompPacket::LEG_SIDE_NO_VALUE = 0xFF;
const int8_t eurexInstrmntLegExecGrpCompPacket::FILL_REF_ID_MIN = 1;
const int8_t eurexInstrmntLegExecGrpCompPacket::FILL_REF_ID_MAX = 100;
const int8_t eurexInstrmntLegExecGrpCompPacket::FILL_REF_ID_NO_VALUE = 0xFF;
const char eurexInstrmntLegExecGrpCompPacket::PAD2_NO_VALUE[2] = {0x00, 0x00};
const size_t eurexInstrmntLegExecGrpCompPacket::PAD2_MAX_LENGTH = 2;


} // namespace neueda

#endif // EUREX_INSTRMNTLEGEXECGRPCOMP_PACKET_H
