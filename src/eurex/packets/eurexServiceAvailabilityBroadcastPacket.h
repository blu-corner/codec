 ...
 */
#ifndef EUREX_SERVICEAVAILABILITYBROADCAST_PACKET_H
#define EUREX_SERVICEAVAILABILITYBROADCAST_PACKET_H
...
class eurexServiceAvailabilityBroadcastPacket
{
    public:
        // no value constants
        ...
        // constructor
        eurexServiceAvailabilityBroadcastPacket ()
        {
        ...
        codecState serialize (void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mMatchingEngineTradeDate, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mTradeManagerTradeDate, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mApplSeqTradeDate, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mT7EntryServiceTradeDate, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mT7EntryServiceRtmTradeDate, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mPartitionID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mMatchingEngineStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mTradeManagerStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mApplSeqStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mT7EntryServiceStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mT7EntryServiceRtmStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        codecState deserialize (const void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mMatchingEngineTradeDate, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mTradeManagerTradeDate, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mApplSeqTradeDate, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mT7EntryServiceTradeDate, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mT7EntryServiceRtmTradeDate, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mPartitionID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mMatchingEngineStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mTradeManagerStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mApplSeqStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mT7EntryServiceStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mT7EntryServiceRtmStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
...
const int32_t eurexServiceAvailabilityBroadcastPacket::MATCHING_ENGINE_TRADE_DATE_MIN = 0;
const int32_t eurexServiceAvailabilityBroadcastPacket::MATCHING_ENGINE_TRADE_DATE_MAX = 99991231;
const int32_t eurexServiceAvailabilityBroadcastPacket::MATCHING_ENGINE_TRADE_DATE_NO_VALUE = 0xFFFFFFFF;
const int32_t eurexServiceAvailabilityBroadcastPacket::TRADE_MANAGER_TRADE_DATE_MIN = 0;
const int32_t eurexServiceAvailabilityBroadcastPacket::TRADE_MANAGER_TRADE_DATE_MAX = 99991231;
const int32_t eurexServiceAvailabilityBroadcastPacket::TRADE_MANAGER_TRADE_DATE_NO_VALUE = 0xFFFFFFFF;
const int32_t eurexServiceAvailabilityBroadcastPacket::APPL_SEQ_TRADE_DATE_MIN = 0;
const int32_t eurexServiceAvailabilityBroadcastPacket::APPL_SEQ_TRADE_DATE_MAX = 99991231;
const int32_t eurexServiceAvailabilityBroadcastPacket::APPL_SEQ_TRADE_DATE_NO_VALUE = 0xFFFFFFFF;
const int32_t eurexServiceAvailabilityBroadcastPacket::T7ENTRY_SERVICE_TRADE_DATE_MIN = 0;
const int32_t eurexServiceAvailabilityBroadcastPacket::T7ENTRY_SERVICE_TRADE_DATE_MAX = 99991231;
const int32_t eurexServiceAvailabilityBroadcastPacket::T7ENTRY_SERVICE_TRADE_DATE_NO_VALUE = 0xFFFFFFFF;
const int32_t eurexServiceAvailabilityBroadcastPacket::T7ENTRY_SERVICE_RTM_TRADE_DATE_MIN = 0;
const int32_t eurexServiceAvailabilityBroadcastPacket::T7ENTRY_SERVICE_RTM_TRADE_DATE_MAX = 99991231;
const int32_t eurexServiceAvailabilityBroadcastPacket::T7ENTRY_SERVICE_RTM_TRADE_DATE_NO_VALUE = 0xFFFFFFFF;
const uint16_t eurexServiceAvailabilityBroadcastPacket::PARTITION_ID_MIN = 0;
const uint16_t eurexServiceAvailabilityBroadcastPacket::PARTITION_ID_MAX = 65534;
const uint16_t eurexServiceAvailabilityBroadcastPacket::PARTITION_ID_NO_VALUE = 0xFFFF;
const int8_t eurexServiceAvailabilityBroadcastPacket::MATCHING_ENGINE_STATUS_MIN = 0;
const int8_t eurexServiceAvailabilityBroadcastPacket::MATCHING_ENGINE_STATUS_MAX = 1;
const int8_t eurexServiceAvailabilityBroadcastPacket::MATCHING_ENGINE_STATUS_NO_VALUE = 0xFF;
const int8_t eurexServiceAvailabilityBroadcastPacket::TRADE_MANAGER_STATUS_MIN = 0;
const int8_t eurexServiceAvailabilityBroadcastPacket::TRADE_MANAGER_STATUS_MAX = 1;
const int8_t eurexServiceAvailabilityBroadcastPacket::TRADE_MANAGER_STATUS_NO_VALUE = 0xFF;
const int8_t eurexServiceAvailabilityBroadcastPacket::APPL_SEQ_STATUS_MIN = 0;
const int8_t eurexServiceAvailabilityBroadcastPacket::APPL_SEQ_STATUS_MAX = 1;
const int8_t eurexServiceAvailabilityBroadcastPacket::APPL_SEQ_STATUS_NO_VALUE = 0xFF;
const int8_t eurexServiceAvailabilityBroadcastPacket::T7ENTRY_SERVICE_STATUS_MIN = 0;
const int8_t eurexServiceAvailabilityBroadcastPacket::T7ENTRY_SERVICE_STATUS_MAX = 1;
const int8_t eurexServiceAvailabilityBroadcastPacket::T7ENTRY_SERVICE_STATUS_NO_VALUE = 0xFF;
const int8_t eurexServiceAvailabilityBroadcastPacket::T7ENTRY_SERVICE_RTM_STATUS_MIN = 0;
const int8_t eurexServiceAvailabilityBroadcastPacket::T7ENTRY_SERVICE_RTM_STATUS_MAX = 1;
const int8_t eurexServiceAvailabilityBroadcastPacket::T7ENTRY_SERVICE_RTM_STATUS_NO_VALUE = 0xFF;
const char eurexServiceAvailabilityBroadcastPacket::PAD5_NO_VALUE[5] = {0x00, 0x00, 0x00, 0x00, 0x00};
const size_t eurexServiceAvailabilityBroadcastPacket::PAD5_MAX_LENGTH = 5;


} // namespace neueda

#endif // EUREX_SERVICEAVAILABILITYBROADCAST_PACKET_H
