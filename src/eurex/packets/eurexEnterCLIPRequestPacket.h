    ...
    public:
        ...
        size_t getRawSize () const
        {
            size_t result = mMessageHeaderIn.getRawSize()
                ...
                + sizeof (mPad4)
                + eurex::getRawSize (mCrossRequestSideGrp)
                + eurex::getRawSize (mSideCrossLegGrp);
            return result;
        }
        ...
        codecState serialize (void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mOrderQtyDisclosureInstruction, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mRootPartyContraFirm, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mRootPartyContraTrader, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        codecState deserialize (const void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mOrderQtyDisclosureInstruction, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mRootPartyContraFirm, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mRootPartyContraTrader, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        string toString () const
        {
            ...
            sss << "EnterCLIPRequest::"
                ...
                << "[Pad4=" << getPad4 () << "],"
                << "[CrossRequestSideGrp=" << eurex::toString (getCrossRequestSideGrp ()) << "],"
                << "[SideCrossLegGrp=" << eurex::toString (getSideCrossLegGrp ()) << "]";
            return sss.str();
        }
};
