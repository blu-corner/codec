 ...
 */
#ifndef EUREX_QUOTEENTRYACKGRPCOMP_PACKET_H
#define EUREX_QUOTEENTRYACKGRPCOMP_PACKET_H
...
class eurexQuoteEntryAckGrpCompPacket
{
    public:
        // no value constants
        ...
        // constructor
        eurexQuoteEntryAckGrpCompPacket ()
        {
        ...
        codecState serialize (void *buf, size_t len, size_t &used)
        {
            ...
            state = eurex::serialize (mBidCxlSize, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mOfferCxlSize, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mQuoteEntryRejectReason, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mQuoteEntryStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        codecState deserialize (const void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mBidCxlSize, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mOfferCxlSize, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mQuoteEntryRejectReason, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mQuoteEntryStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
...
const int64_t eurexQuoteEntryAckGrpCompPacket::SECURITY_ID_MIN = -9223372036854775807L;
const int64_t eurexQuoteEntryAckGrpCompPacket::SECURITY_ID_MAX = 9223372036854775807L;
const int64_t eurexQuoteEntryAckGrpCompPacket::SECURITY_ID_NO_VALUE = 0x8000000000000000;
const double eurexQuoteEntryAckGrpCompPacket::BID_CXL_SIZE_MIN = -922337203685477.5807;
const double eurexQuoteEntryAckGrpCompPacket::BID_CXL_SIZE_MAX = 922337203685477.5807;
const int64_t eurexQuoteEntryAckGrpCompPacket::BID_CXL_SIZE_NO_VALUE = 0x8000000000000000;
const double eurexQuoteEntryAckGrpCompPacket::OFFER_CXL_SIZE_MIN = -922337203685477.5807;
const double eurexQuoteEntryAckGrpCompPacket::OFFER_CXL_SIZE_MAX = 922337203685477.5807;
const int64_t eurexQuoteEntryAckGrpCompPacket::OFFER_CXL_SIZE_NO_VALUE = 0x8000000000000000;
const int32_t eurexQuoteEntryAckGrpCompPacket::QUOTE_ENTRY_REJECT_REASON_MIN = 0;
const int32_t eurexQuoteEntryAckGrpCompPacket::QUOTE_ENTRY_REJECT_REASON_MAX = 65535;
const int32_t eurexQuoteEntryAckGrpCompPacket::QUOTE_ENTRY_REJECT_REASON_NO_VALUE = 0xFFFFFFFF;
const int8_t eurexQuoteEntryAckGrpCompPacket::QUOTE_ENTRY_STATUS_MIN = 0;
const int8_t eurexQuoteEntryAckGrpCompPacket::QUOTE_ENTRY_STATUS_MAX = 100;
const int8_t eurexQuoteEntryAckGrpCompPacket::QUOTE_ENTRY_STATUS_NO_VALUE = 0xFF;
const char eurexQuoteEntryAckGrpCompPacket::PAD3_NO_VALUE[3] = {0x00, 0x00, 0x00};
const size_t eurexQuoteEntryAckGrpCompPacket::PAD3_MAX_LENGTH = 3;


} // namespace neueda

#endif // EUREX_QUOTEENTRYACKGRPCOMP_PACKET_H
