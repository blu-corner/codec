 ...
 */
#ifndef EUREX_RESPONSEHEADERMECOMP_PACKET_H
#define EUREX_RESPONSEHEADERMECOMP_PACKET_H
...
class eurexResponseHeaderMECompPacket
{
    public:
        // no value constants
        ...
        // constructor
        eurexResponseHeaderMECompPacket ()
        {
        ...
        {
            codecState state;
            state = eurex::serialize (mRequestTime, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mTrdRegTSTimeIn, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mTrdRegTSTimeOut, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mResponseIn, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mApplID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mApplMsgID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mLastFragment, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        {
            codecState state;
            state = eurex::deserialize (mRequestTime, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mTrdRegTSTimeIn, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mTrdRegTSTimeOut, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mResponseIn, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mApplID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mApplMsgID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mLastFragment, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            return GW_CODEC_SUCCESS;
        }
...
const uint64_t eurexResponseHeaderMECompPacket::REQUEST_TIME_MIN = 0UL;
const uint64_t eurexResponseHeaderMECompPacket::REQUEST_TIME_MAX = 18446744073709551614UL;
const uint64_t eurexResponseHeaderMECompPacket::REQUEST_TIME_NO_VALUE = 0xFFFFFFFFFFFFFFFF;
const uint64_t eurexResponseHeaderMECompPacket::TRD_REG_TSTIME_IN_MIN = 0UL;
const uint64_t eurexResponseHeaderMECompPacket::TRD_REG_TSTIME_IN_MAX = 18446744073709551614UL;
const uint64_t eurexResponseHeaderMECompPacket::TRD_REG_TSTIME_IN_NO_VALUE = 0xFFFFFFFFFFFFFFFF;
const uint64_t eurexResponseHeaderMECompPacket::TRD_REG_TSTIME_OUT_MIN = 0UL;
const uint64_t eurexResponseHeaderMECompPacket::TRD_REG_TSTIME_OUT_MAX = 18446744073709551614UL;
const uint64_t eurexResponseHeaderMECompPacket::TRD_REG_TSTIME_OUT_NO_VALUE = 0xFFFFFFFFFFFFFFFF;
const uint64_t eurexResponseHeaderMECompPacket::RESPONSE_IN_MIN = 0UL;
const uint64_t eurexResponseHeaderMECompPacket::RESPONSE_IN_MAX = 18446744073709551614UL;
const uint64_t eurexResponseHeaderMECompPacket::RESPONSE_IN_NO_VALUE = 0xFFFFFFFFFFFFFFFF;
const uint64_t eurexResponseHeaderMECompPacket::SENDING_TIME_MIN = 0UL;
const uint64_t eurexResponseHeaderMECompPacket::SENDING_TIME_MAX = 18446744073709551614UL;
const uint64_t eurexResponseHeaderMECompPacket::SENDING_TIME_NO_VALUE = 0xFFFFFFFFFFFFFFFF;
const uint32_t eurexResponseHeaderMECompPacket::MSG_SEQ_NUM_MIN = 0;
const uint32_t eurexResponseHeaderMECompPacket::MSG_SEQ_NUM_MAX = 4294967294;
const uint32_t eurexResponseHeaderMECompPacket::MSG_SEQ_NUM_NO_VALUE = 0xFFFFFFFF;
const uint16_t eurexResponseHeaderMECompPacket::PARTITION_ID_MIN = 0;
const uint16_t eurexResponseHeaderMECompPacket::PARTITION_ID_MAX = 65534;
const uint16_t eurexResponseHeaderMECompPacket::PARTITION_ID_NO_VALUE = 0xFFFF;
const int8_t eurexResponseHeaderMECompPacket::APPL_ID_MIN = 0;
const int8_t eurexResponseHeaderMECompPacket::APPL_ID_MAX = 10;
const int8_t eurexResponseHeaderMECompPacket::APPL_ID_NO_VALUE = 0xFF;
const char eurexResponseHeaderMECompPacket::APPL_MSG_ID_NO_VALUE[16] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const size_t eurexResponseHeaderMECompPacket::APPL_MSG_ID_MAX_LENGTH = 16;
const int8_t eurexResponseHeaderMECompPacket::LAST_FRAGMENT_MIN = 0;
const int8_t eurexResponseHeaderMECompPacket::LAST_FRAGMENT_MAX = 1;
const int8_t eurexResponseHeaderMECompPacket::LAST_FRAGMENT_NO_VALUE = 0xFF;


} // namespace neueda

#endif // EUREX_RESPONSEHEADERMECOMP_PACKET_H
