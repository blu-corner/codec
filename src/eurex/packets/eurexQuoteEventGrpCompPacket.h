 ...
 */
#ifndef EUREX_QUOTEEVENTGRPCOMP_PACKET_H
#define EUREX_QUOTEEVENTGRPCOMP_PACKET_H
...
class eurexQuoteEventGrpCompPacket
{
    public:
        // no value constants
        ...
        codecState serialize (void *buf, size_t len, size_t &used)
        {
            ...
            state = eurex::serialize (mQuoteEventPx, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mQuoteEventQty, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mQuoteMsgID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mQuoteEventMatchID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mQuoteEventExecID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mQuoteEventType, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mQuoteEventSide, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mQuoteEventLiquidityInd, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mQuoteEventReason, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        codecState deserialize (const void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mQuoteEventPx, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mQuoteEventQty, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mQuoteMsgID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mQuoteEventMatchID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mQuoteEventExecID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mQuoteEventType, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mQuoteEventSide, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mQuoteEventLiquidityInd, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mQuoteEventReason, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
...
const int64_t eurexQuoteEventGrpCompPacket::SECURITY_ID_MIN = -9223372036854775807L;
const int64_t eurexQuoteEventGrpCompPacket::SECURITY_ID_MAX = 9223372036854775807L;
const int64_t eurexQuoteEventGrpCompPacket::SECURITY_ID_NO_VALUE = 0x8000000000000000;
const double eurexQuoteEventGrpCompPacket::QUOTE_EVENT_PX_MIN = -92233720368.54775807;
const double eurexQuoteEventGrpCompPacket::QUOTE_EVENT_PX_MAX = 92233720368.54775807;
const int64_t eurexQuoteEventGrpCompPacket::QUOTE_EVENT_PX_NO_VALUE = 0x8000000000000000;
const double eurexQuoteEventGrpCompPacket::QUOTE_EVENT_QTY_MIN = -922337203685477.5807;
const double eurexQuoteEventGrpCompPacket::QUOTE_EVENT_QTY_MAX = 922337203685477.5807;
const int64_t eurexQuoteEventGrpCompPacket::QUOTE_EVENT_QTY_NO_VALUE = 0x8000000000000000;
const uint64_t eurexQuoteEventGrpCompPacket::QUOTE_MSG_ID_MIN = 0UL;
const uint64_t eurexQuoteEventGrpCompPacket::QUOTE_MSG_ID_MAX = 18446744073709551614UL;
const uint64_t eurexQuoteEventGrpCompPacket::QUOTE_MSG_ID_NO_VALUE = 0xFFFFFFFFFFFFFFFF;
const uint32_t eurexQuoteEventGrpCompPacket::QUOTE_EVENT_MATCH_ID_MIN = 0;
const uint32_t eurexQuoteEventGrpCompPacket::QUOTE_EVENT_MATCH_ID_MAX = 4294967294;
const uint32_t eurexQuoteEventGrpCompPacket::QUOTE_EVENT_MATCH_ID_NO_VALUE = 0xFFFFFFFF;
const int32_t eurexQuoteEventGrpCompPacket::QUOTE_EVENT_EXEC_ID_MIN = -2147483647;
const int32_t eurexQuoteEventGrpCompPacket::QUOTE_EVENT_EXEC_ID_MAX = 2147483647;
const int32_t eurexQuoteEventGrpCompPacket::QUOTE_EVENT_EXEC_ID_NO_VALUE = 0x80000000;
const int8_t eurexQuoteEventGrpCompPacket::QUOTE_EVENT_TYPE_MIN = 0;
const int8_t eurexQuoteEventGrpCompPacket::QUOTE_EVENT_TYPE_MAX = 5;
const int8_t eurexQuoteEventGrpCompPacket::QUOTE_EVENT_TYPE_NO_VALUE = 0xFF;
const int8_t eurexQuoteEventGrpCompPacket::QUOTE_EVENT_SIDE_MIN = 1;
const int8_t eurexQuoteEventGrpCompPacket::QUOTE_EVENT_SIDE_MAX = 2;
const int8_t eurexQuoteEventGrpCompPacket::QUOTE_EVENT_SIDE_NO_VALUE = 0xFF;
const int8_t eurexQuoteEventGrpCompPacket::QUOTE_EVENT_LIQUIDITY_IND_MIN = 1;
const int8_t eurexQuoteEventGrpCompPacket::QUOTE_EVENT_LIQUIDITY_IND_MAX = 4;
const int8_t eurexQuoteEventGrpCompPacket::QUOTE_EVENT_LIQUIDITY_IND_NO_VALUE = 0xFF;
const int8_t eurexQuoteEventGrpCompPacket::QUOTE_EVENT_REASON_MIN = 14;
const int8_t eurexQuoteEventGrpCompPacket::QUOTE_EVENT_REASON_MAX = 21;
const int8_t eurexQuoteEventGrpCompPacket::QUOTE_EVENT_REASON_NO_VALUE = 0xFF;
const char eurexQuoteEventGrpCompPacket::PAD4_NO_VALUE[4] = {0x00, 0x00, 0x00, 0x00};
const size_t eurexQuoteEventGrpCompPacket::PAD4_MAX_LENGTH = 4;


} // namespace neueda

#endif // EUREX_QUOTEEVENTGRPCOMP_PACKET_H
