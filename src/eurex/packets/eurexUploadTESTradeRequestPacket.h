    ...
    public:
        ...
        size_t getRawSize () const
        {
            size_t result = mMessageHeaderIn.getRawSize()
                ...
                + sizeof (mPad4)
                + eurex::getRawSize (mSideAllocExtGrp)
        ...
        codecState serialize (void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mSkipValidations, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mTrdRptStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::serialize (mTradePlatform, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        codecState deserialize (const void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mSkipValidations, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mTrdRptStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eurex::deserialize (mTradePlatform, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        string toString () const
        {
            ...
            sss << "UploadTESTradeRequest::"
                ...
                << "[Pad4=" << getPad4 () << "],"
                << "[SideAllocExtGrp=" << eurex::toString (getSideAllocExtGrp ()) << "],"
