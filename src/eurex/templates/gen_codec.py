...
# generate files
packet_h = os.path.join(packets_dir, 'eurexPackets.h')
compose_packet_h(packet_h, generation, venue)

msg_packet_h = os.path.join(packets_dir, 'eurex{0}Packet.h')
...
codec_h = os.path.join(output_dir, 'eurexCodec.h')
compose_codec_h(codec_h, venue)

codec_cpp = os.path.join(output_dir, 'eurexCodec.cpp')
