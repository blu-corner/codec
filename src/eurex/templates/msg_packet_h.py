 ...
 */
#ifndef EUREX_{MESSAGE}_PACKET_H
#define EUREX_{MESSAGE}_PACKET_H
...
#endif // EUREX_{MESSAGE}_PACKET_H
'''
...
def render_serialize(struct, venue):
	...
	for field in struct.findall('field'):
		...
		if field.attrib['class'] == 'sequence':
			...
			code += '{0}state = eurex::serialize (m{1}, buf, len, used);\n'.format(spaces, field.attrib['name']);
		elif field.attrib['class'] == 'double':
			code += '{0}state = eurex::serialize (m{1}, buf, len, used);\n'.format(spaces, field.attrib['name']);
		elif field.attrib['class'] == 'string':
			code += '{0}state = eurex::serialize (m{1}, buf, len, used);\n'.format(spaces, field.attrib['name']);
		elif field.attrib['class'] == 'varstring':
			code += '{0}state = eurex::serialize (m{1}, {2}, buf, len, used);\n'.format(spaces, field.attrib['name'], get_constname(field.attrib['name'], 'MAX_LENGTH'));
			hasVarText = True
		else:
			raise Exception('unknown class ' + field.attrib['class'])
		if checkState:
	...
	if hasVarText:
		...
		code += '{0}state = eurex::serialize (string (extraPad, \'\\0\'), extraPad, buf, len, used);\n'.format(spaces);
		code += '{0}if (state != GW_CODEC_SUCCESS) return state;\n'. format(spaces)
	format = {'CODE':code}
...
def render_deserialize(struct, venue):
	...
	for field in struct.findall('field'):
		...
		if field.attrib['class'] == 'sequence':
			...
			code += '{0}{1} v{2};\n'.format(spaces, field.attrib['type'], field.attrib['name']);
			code += '{0}state = eurex::deserialize (v{1}, buf, len, used);\n'.format(spaces, field.attrib['name']);
			code += '{0}m{1} = v{1};\n'.format(spaces, field.attrib['name']);
		elif field.attrib['class'] == 'integer':
			code += '{0}state = eurex::deserialize (m{1}, buf, len, used);\n'.format(spaces, field.attrib['name']);
		elif field.attrib['class'] == 'double':
			code += '{0}state = eurex::deserialize (m{1}, buf, len, used);\n'.format(spaces, field.attrib['name']);
		elif field.attrib['class'] == 'string':
			code += '{0}state = eurex::deserialize (m{1}, buf, len, used);\n'.format(spaces, field.attrib['name']);
		elif field.attrib['class'] == 'varstring':
			code += '{0}state = eurex::deserialize (m{1}, m{2}, buf, len, used);\n'.format(spaces, field.attrib['name'], field.attrib['counter']);
			hasVarText = True
		else:
			raise Exception('unknown class ' + field.attrib['class'])
		if checkState:
	...
	if hasVarText:
		...
		code += '{0}state = eurex::deserialize (stringPad, extraPad, buf, len, used);\n'.format(spaces);
		code += '{0}if (state != GW_CODEC_SUCCESS) return state;\n'. format(spaces)
	format = {'CODE':code}
...
def render_to_string(struct, venue):
	...
	for field in struct.findall('field'):
		if field.attrib['class'] == 'sequence':
			code += '{0}<< "[{1}=" << eurex::toString (get{1} ()) << "]'.format(prefix, field.attrib['name']);
		elif field.attrib['class'] == 'group':
