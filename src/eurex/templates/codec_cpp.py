...
CODEC_CPP_GET_GROUP = '''\
static void
get{GROUP} (cdr &d, const eurex{GROUP}Packet& packet)
{{
{CODE}
}}
'''
...
void
eurexCodec::get{MESSAGE} (cdr &d, const void *buf, size_t len, size_t& used)
{{
    eurex{MESSAGE}Packet packet;
    codecState state = packet.deserialize (buf, len, used);
    if (state != GW_CODEC_SUCCESS)
        throw eurex::codecError(state, "cannot deserialize {MESSAGE}");
...
CODEC_CPP_PUT_GROUP = '''\
static void
put{GROUP} (const cdr &d, eurex{GROUP}Packet& packet)
{{
{CODE}
}}
'''
...
void
eurexCodec::put{MESSAGE} (const cdr &d, void *buf, size_t len, size_t& used)
{{
    eurex{MESSAGE}Packet packet;
{CODE}
    codecState state = packet.serialize (buf, len, used);
    if (state != GW_CODEC_SUCCESS)
        throw eurex::codecError(state, "cannot serialize {MESSAGE}");
}}
'''
...
{SWITCH}
            default:
                ...
                throw eurex::codecError (GW_CODEC_ERROR, ss.str ());
        }}
    }}
    catch (eurex::codecError &e)
    {{
...
def render_get_fields(group, venue):
	...
	for field in group.findall('field'):
		if field.attrib['class'] == 'sequence':
			result += '{0}const vector<{1}>& v{2} = packet.get{2} ();\n'.format(spaces, get_typename(field.attrib['type']), field.attrib['name'])
			...
			result += '{0}eurex::setInteger<{1}, {2}, &{2}::get{3}, &{2}::is{3}Valid> (d, {3}, packet);\n'.format(spaces, field.attrib['type'], get_typename(group.attrib['name']), field.attrib['name'])
		elif field.attrib['class'] == 'double':
			result += '{0}eurex::setDouble<{1}, &{1}::get{2}, &{1}::is{2}Valid> (d, {2}, packet);\n'.format(spaces, get_typename(group.attrib['name']), field.attrib['name'])
		elif field.attrib['class'] == 'string':
			result += '{0}eurex::setString<{1}, &{1}::get{2}, &{1}::is{2}Valid> (d, {2}, packet);\n'.format(spaces, get_typename(group.attrib['name']), field.attrib['name'])
...
def render_put_fields(struct, venue, number_of_spaces=4):
	...
	for field in struct.findall('field'):
		if field.attrib['class'] == 'sequence':
			...
			result += '{0}{{\n'.format(spaces)
			result += '{0}    {1} group;\n'.format(spaces, get_typename(field.attrib['type']))
			...
			result += '{0}eurex::getInteger<{1}, {2}, &{2}::set{3}, &{2}::reset{3}> (d, {3}, packet);\n'.format(spaces, field.attrib['type'], get_typename(struct.attrib['name']), field.attrib['name'])
		elif field.attrib['class'] == 'double':
			result += '{0}eurex::getDouble<{1}, &{1}::set{2}, &{1}::reset{2}> (d, {2}, packet);\n'.format(spaces, get_typename(struct.attrib['name']), field.attrib['name'])
		elif field.attrib['class'] == 'string':
			result += '{0}eurex::getString<{1}, &{1}::set{2}, &{1}::reset{2}> (d, {2}, packet);\n'.format(spaces, get_typename(struct.attrib['name']), field.attrib['name'])
		elif field.attrib['class'] == 'varstring':
			result += '{0}eurex::getString<{1}, &{1}::set{2}, &{1}::reset{2}> (d, {2}, packet);\n'.format(spaces, get_typename(struct.attrib['name']), field.attrib['name'])
		else:
			raise Exception('unknown class ' + field.attrib['class'])
...
def render_decode(venue):
	...
	return CODEC_CPP_DECODE.format(SWITCH=switch[:-1])
...
def render_encode(venue):
	...
	return CODEC_CPP_ENCODE.format(SWITCH=switch[:-1])
