    ...
    public:
        ...
        size_t getRawSize () const
        {
            size_t result = mMessageHeaderIn.getRawSize()
                ...
                + sizeof (mPad1)
                + eti::getRawSize (mSecurityStatusEventGrp);
            return result;
        }
        ...
        codecState serialize (void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::serialize (mSecurityStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::serialize (mSoldOutIndicator, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        codecState deserialize (const void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::deserialize (mSecurityStatus, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::deserialize (mSoldOutIndicator, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        string toString () const
        {
            ...
            sss << "IssuerSecurityStateChangeRequest::"
                ...
                << "[Pad1=" << getPad1 () << "],"
                << "[SecurityStatusEventGrp=" << eti::toString (getSecurityStatusEventGrp ()) << "]";
            return sss.str();
        }
};
