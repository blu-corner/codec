    ...
    public:
        ...
        size_t getRawSize () const
        {
            size_t result = mMessageHeaderOut.getRawSize()
                ...
                + sizeof (mPad4)
                + eti::getRawSize (mRiskLimitsRptGrp);
            return result;
        }
        ...
        codecState serialize (void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::serialize (mRiskLimitReportID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::serialize (mMarketSegmentID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::serialize (mNoRiskLimits, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        codecState deserialize (const void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::deserialize (mRiskLimitReportID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::deserialize (mMarketSegmentID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::deserialize (mNoRiskLimits, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        string toString () const
        {
            ...
            sss << "PreTradeRiskLimitResponse::"
                ...
                << "[Pad4=" << getPad4 () << "],"
                << "[RiskLimitsRptGrp=" << eti::toString (getRiskLimitsRptGrp ()) << "]";
            return sss.str();
        }
};
