...
class eurexOrderBookItemGrpCompPacket
{
    public:
        ...
        codecState serialize (void *buf, size_t len, size_t &used)
        {
            ...
            state = eti::serialize (mBestBidPx, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::serialize (mBestBidSize, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::serialize (mBestOfferPx, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::serialize (mBestOfferSize, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::serialize (mMDBookType, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::serialize (mMDSubBookType, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        codecState deserialize (const void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::deserialize (mBestBidPx, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::deserialize (mBestBidSize, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::deserialize (mBestOfferPx, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::deserialize (mBestOfferSize, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::deserialize (mMDBookType, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::deserialize (mMDSubBookType, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
