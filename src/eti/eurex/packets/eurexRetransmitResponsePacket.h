 ...
 */
#ifndef EUREX_RETRANSMITRESPONSE_PACKET_H
#define EUREX_RETRANSMITRESPONSE_PACKET_H
...
class eurexRetransmitResponsePacket
{
    public:
        // no value constants
        ...
        codecState serialize (void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::serialize (mApplEndSeqNum, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::serialize (mRefApplLastSeqNum, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::serialize (mApplTotalMessageCount, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        codecState deserialize (const void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::deserialize (mApplEndSeqNum, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::deserialize (mRefApplLastSeqNum, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::deserialize (mApplTotalMessageCount, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
...
const uint64_t eurexRetransmitResponsePacket::APPL_END_SEQ_NUM_MIN = 0UL;
const uint64_t eurexRetransmitResponsePacket::APPL_END_SEQ_NUM_MAX = 18446744073709551614UL;
const uint64_t eurexRetransmitResponsePacket::APPL_END_SEQ_NUM_NO_VALUE = 0xFFFFFFFFFFFFFFFF;
const uint64_t eurexRetransmitResponsePacket::REF_APPL_LAST_SEQ_NUM_MIN = 0UL;
const uint64_t eurexRetransmitResponsePacket::REF_APPL_LAST_SEQ_NUM_MAX = 18446744073709551614UL;
const uint64_t eurexRetransmitResponsePacket::REF_APPL_LAST_SEQ_NUM_NO_VALUE = 0xFFFFFFFFFFFFFFFF;
const uint16_t eurexRetransmitResponsePacket::APPL_TOTAL_MESSAGE_COUNT_MIN = 0;
const uint16_t eurexRetransmitResponsePacket::APPL_TOTAL_MESSAGE_COUNT_MAX = 65534;
const uint16_t eurexRetransmitResponsePacket::APPL_TOTAL_MESSAGE_COUNT_NO_VALUE = 0xFFFF;
const char eurexRetransmitResponsePacket::PAD6_NO_VALUE[6] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const size_t eurexRetransmitResponsePacket::PAD6_MAX_LENGTH = 6;


} // namespace neueda

#endif // EUREX_RETRANSMITRESPONSE_PACKET_H
