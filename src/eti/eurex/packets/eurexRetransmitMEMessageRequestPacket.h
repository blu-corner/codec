 ...
 */
#ifndef EUREX_RETRANSMITMEMESSAGEREQUEST_PACKET_H
#define EUREX_RETRANSMITMEMESSAGEREQUEST_PACKET_H
...
class eurexRetransmitMEMessageRequestPacket
{
    public:
        // no value constants
        ...
        // constructor
        eurexRetransmitMEMessageRequestPacket ()
        {
        ...
        codecState serialize (void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::serialize (mSubscriptionScope, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::serialize (mPartitionID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::serialize (mRefApplID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::serialize (mApplBegMsgID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::serialize (mApplEndMsgID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
        ...
        codecState deserialize (const void *buf, size_t len, size_t &used)
        {
            ...
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::deserialize (mSubscriptionScope, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::deserialize (mPartitionID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::deserialize (mRefApplID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::deserialize (mApplBegMsgID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
            state = eti::deserialize (mApplEndMsgID, buf, len, used);
            if (state != GW_CODEC_SUCCESS) return state;
...
const uint32_t eurexRetransmitMEMessageRequestPacket::SUBSCRIPTION_SCOPE_MIN = 0;
const uint32_t eurexRetransmitMEMessageRequestPacket::SUBSCRIPTION_SCOPE_MAX = 4294967294;
const uint32_t eurexRetransmitMEMessageRequestPacket::SUBSCRIPTION_SCOPE_NO_VALUE = 0xFFFFFFFF;
const uint16_t eurexRetransmitMEMessageRequestPacket::PARTITION_ID_MIN = 0;
const uint16_t eurexRetransmitMEMessageRequestPacket::PARTITION_ID_MAX = 65534;
const uint16_t eurexRetransmitMEMessageRequestPacket::PARTITION_ID_NO_VALUE = 0xFFFF;
const int8_t eurexRetransmitMEMessageRequestPacket::REF_APPL_ID_MIN = 0;
const int8_t eurexRetransmitMEMessageRequestPacket::REF_APPL_ID_MAX = 10;
const int8_t eurexRetransmitMEMessageRequestPacket::REF_APPL_ID_NO_VALUE = 0xFF;
const char eurexRetransmitMEMessageRequestPacket::APPL_BEG_MSG_ID_NO_VALUE[16] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const size_t eurexRetransmitMEMessageRequestPacket::APPL_BEG_MSG_ID_MAX_LENGTH = 16;
const char eurexRetransmitMEMessageRequestPacket::APPL_END_MSG_ID_NO_VALUE[16] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const size_t eurexRetransmitMEMessageRequestPacket::APPL_END_MSG_ID_MAX_LENGTH = 16;
const char eurexRetransmitMEMessageRequestPacket::PAD1_NO_VALUE[1] = {0x00};
const size_t eurexRetransmitMEMessageRequestPacket::PAD1_MAX_LENGTH = 1;


} // namespace neueda

#endif // EUREX_RETRANSMITMEMESSAGEREQUEST_PACKET_H
